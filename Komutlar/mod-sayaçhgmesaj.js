...
exports.run = async (client, message, args, prefix) => {
  ...
  dbl.hasVoted(message.author.id).then(voted => {
    ...
    if(!args[0]) {
      ...
      .setColor(ee.color)
      .setDescription(`${ee.e.unlem} Merhaba, Öncelikle sayaç mesajı ayarlamak istiyorsan eğer bir değer belirtmelisin! örnek: **${p}sayaçhgmesaj ayarla** veya **${p}sayaçhgmesaj sıfırla**!`)
    ...
    if(args[0] === "ayarla") {
     ...
     const embedd = new Discord.MessageEmbed()
      .setColor(ee.color)
      .setDescription(`${ee.e.no} Merhaba, Sayaç Mesajı ayarlamak istiyorsan eğer bir mesaj belirlemelisin! örnek: **${prefix}sayaçhgmesaj ayarla <mesaj>**`)
    ...
    const embedd = new Discord.MessageEmbed()
      .setColor(ee.color)
      .setDescription(`${ee.e.yes} Merhaba, Başarılı bir şekilde sayaç hoş geldin mesajını ayarladım!\n Ayarlanmış değerler; Mesaj: ${mesaj}`)
      return message.channel.send(embedd)	
    }
